...
long mini_test_strtol(const char *nptr, char **endptr, int base);
...
static int compare_case(const char *text, int base)
{
    ...
    *mini_errno = 7;
    ...
    host_value = strtol(text, &host_end, base);
    mini_value = mini_test_strtol(text, &mini_end, base);
    ...
    if (host_value != mini_value || host_offset != mini_offset ||
        errno != *mini_errno) {
        fprintf(stderr,
                "mismatch: text=[%s] base=%d host=(%ld,%ld,%d) mini=(%ld,%ld,%d)\n",
...
int main(void)
{
    ...
    puts("strtol differential passed");
    return 0;
}
