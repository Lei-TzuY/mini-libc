...
int main(void)
{
    ...
    static const char utf8_narrow_text[] = {
        (char)0xc2, (char)0xa2, (char)0xe2, (char)0x82, (char)0xac, 0
    };
    static const char invalid_utf8_text[] = {(char)0xc2, 'X', 0};
    ...
    errno = ERANGE;
    count = swprintf(formatted, 64U, L"[%.1s]", utf8_narrow_text);
    if (count != 3 || wcscmp(formatted, utf8_narrow_expected) != 0 ||
        errno != ERANGE) {
        return fail((FILE *)0, 71);
    }
    errno = 0;
    if (swprintf(formatted, 64U, L"%s", invalid_utf8_text) != EOF ||
        errno != EILSEQ) {
        return fail((FILE *)0, 72);
    }
