...
wint_t __mini_wide_read_character_unlocked(FILE *stream)
{
    ...
    for (;;) {
        ...
        converted = __mini_mbrtowc_mode(&wc, (const char *)&byte, 1U,
                                        &state, utf8);
...
static wint_t wide_write_character_unlocked(wchar_t wc, FILE *stream)
{
    ...
    converted = __mini_wcrtomb_mode(bytes, wc, &state, utf8);
    if (converted == (size_t)-1) {
        return wide_error(stream, errno);
    }
    if (__mini_stdio_write(stream, (const unsigned char *)bytes, converted) !=
        converted) {
        return WEOF;
    }
    return (wint_t)wc;
...
wint_t __mini_wide_unget_character_unlocked(wint_t wc, FILE *stream)
{
    ...
    if (wc == WEOF || stream == (FILE *)0) {
        return wide_error(stream, EINVAL);
    }

    utf8 = (stream->state & MINI_FILE_WIDE_UTF8) != 0U;
    converted = __mini_wcrtomb_mode(bytes, (wchar_t)wc, &state, utf8);
    if (converted == (size_t)-1) {
        return wide_error(stream, errno);
    }
    if (__mini_stdio_pushback_unlocked(stream,
            (const unsigned char *)bytes, converted) == EOF) {
        return WEOF;
    }
    return wc;
}
...
static wint_t wide_get_unlocked(FILE *stream)
{
    ...
    return __mini_wide_read_character_unlocked(stream);
}
...
wchar_t *fgetws(wchar_t *restrict s, int n, FILE *restrict stream)
{
    ...
    if (s == (wchar_t *)0 || n <= 0) {
        ...
        while (length + 1U < (size_t)n) {
            wint_t wc = __mini_wide_read_character_unlocked(stream);
...
wint_t ungetwc(wint_t wc, FILE *stream)
{
    ...
    if (__mini_stdio_require_wide(stream) == EOF) {
        ...
        result = __mini_wide_unget_character_unlocked(wc, stream);
    }
    __mini_stdio_unlock();
    return result;
}
