...
static atomic_int mini_once_lock_word = ATOMIC_VAR_INIT(0);

static void once_lock(void)
{
    while (atomic_exchange(&mini_once_lock_word, 1) != 0) {
        (void)mini_sys_futex((volatile int *)&mini_once_lock_word,
                             MINI_FUTEX_WAIT, 1, (const void *)0,
                             (volatile int *)0, 0);
    }
}
...
{
    if (atomic_exchange(&mini_once_lock_word, 0) != 0) {
        (void)mini_sys_futex((volatile int *)&mini_once_lock_word,
                             MINI_FUTEX_WAKE, 1, (const void *)0,
                             (volatile int *)0, 0);
    }
}
...
void call_once(once_flag *flag, void (*func)(void))
{
    for (;;) {
        ...
        once_lock();
        state = atomic_load(&flag->__state);
        if (state == MINI_ONCE_COMPLETE) {
            once_unlock();
            return;
        }
        if (state == MINI_ONCE_UNINITIALIZED) {
            atomic_store(&flag->__state, MINI_ONCE_RUNNING);
            once_unlock();

            func();

            once_lock();
            atomic_store(&flag->__state, MINI_ONCE_COMPLETE);
            once_unlock();
